  ...
  async generateOrderName(salespersonId = null) {
    try {
      ...
      if (targetSalespersonId === this.pos.user.id) {
        ...
        currentOrder &&
        currentOrder.getSalesperson() &&
        currentOrder.getSalesperson().id === targetSalespersonId
      ) {
  ...
  async createOrder() {
    ...
    try {
      ...
      const orderLinesData = orderlines.map((line) => ({
        ...
        subtotal: line.priceIncl,
        tax_amount: line.priceIncl - line.priceExcl,
      }));
      ...
      const orderData = {
        ...
        amount_total: currentOrder.priceIncl,
        amount_untaxed: currentOrder.priceExcl,
        amount_tax: currentOrder.amountTaxes,
