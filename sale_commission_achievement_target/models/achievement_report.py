...
class SaleCommissionAchievementReport(models.Model):
    ...
    def _get_config_params_safely(self):
        ...
        try:
            ...
            mandatory_enabled_str = IrConfigParam.get_param(
                'sale_commission_achievement_target.mandatory_category_enabled', 
                'False'
            )
            mandatory_category_id_str = IrConfigParam.get_param(
                'sale_commission_achievement_target.mandatory_category_id', 
                False
            )
            ...
            try:
                mandatory_enabled_bool = str(mandatory_enabled_str).strip().lower() in ('true', '1', 'yes')
            except (AttributeError, ValueError) as e:
                _logger.warning("Error al procesar mandatory_category_enabled: %s. Usando False por defecto.", e)
            ...
            if mandatory_category_id_str:
                try:
                    ...
                    if cleaned_id.isdigit() and int(cleaned_id) > 0:
                        ...
                        category_exists = self.env['product.public.category'].sudo().search_count(
                            [('id', '=', mandatory_category_id_int)]
                        )
            ...
            _logger.error("Error crítico al obtener parámetros de configuración: %s", e, exc_info=True)
    ...
    def _rate_to_case(self, rates):
        ...
        if mandatory_enabled_bool and mandatory_category_id_int:
            ...
            if priority_category_id and mandatory_category_id_int != priority_category_id:
                ...
                priority_exclusion_mandatory = (
                    ...
                    "                                      AND ppcrel_priority_mand.product_public_category_id = " + validated_priority_id + "\n"
                    "                                  )\n                                "
                )
            ...
            mandatory_check_sales = (
                ...
                "                              AND scpa_check.public_categ_id = " + validated_mandatory_id + "\n"
                ...
                "                                      AND ppcrel.product_public_category_id = " + validated_mandatory_id + "\n"
                "                                  )" + priority_exclusion_mandatory + 
                ...
                "                              ) >= (scpa_check.target_amount * " + validated_percentage + ")\n"
                "                        ) THEN 0\n            "
            )
        ...
        if is_sales:
            ...
            if priority_category_id:
                ...
                priority_exclusion = (
                    ...
                    "                              AND (\n"
                    "                                scpa.public_categ_id = " + validated_priority_id + " OR\n"
                    "                                NOT EXISTS (\n"
                    ...
                    "                                      AND ppcrel_priority.product_public_category_id = " + validated_priority_id + "\n"
                    "                                )\n"
                    "                              )\n                "
                )
            ...
            CASE 
                ...
                ELSE 0 
            END AS %%s
            """ % (mandatory_check_sales, priority_exclusion)
        elif is_invoices:
            # Construir la condición de exclusión de productos en categoría prioritaria
            ...
            if priority_category_id:
                ...
                priority_exclusion_inv = (
                    "\n                              -- Lógica de prioridad: Si este achievement NO es categoría prioritaria, excluir productos que estén en categoría prioritaria\n"
                    "                              AND (\n"
                    "                                scpa.public_categ_id = " + validated_priority_id_inv + " OR\n"
                    "                                NOT EXISTS (\n"
                    "                                    SELECT 1 FROM product_public_category_product_template_rel ppcrel_priority\n"
                    "                                    WHERE ppcrel_priority.product_template_id = pt_check.id\n"
                    "                                      AND ppcrel_priority.product_public_category_id = " + validated_priority_id_inv + "\n"
                    "                                )\n"
                    "                              )\n                "
                )
            ...
            CASE 
                ...
                ELSE 0 
            END AS %%s
            """ % (mandatory_check_invoices, priority_exclusion_inv)
        else:
